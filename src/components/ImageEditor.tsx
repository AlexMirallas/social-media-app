...
function ImageEditor({onClose, previewURL, settings, setSettings}: {
    ...
    const changeSensitive = (sensitive: boolean) => {
        ...
        <div className="fixed w-screen h-screen left-0 top-0 bg-black bg-opacity-75 z-10 flex items-center justify-center">
            <div className="bg-black rounded-xl p-12 flex flex-col gap-4">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <button className="py-2 cursor-pointer px-4 bg-white font-bold text-black rounded-full" onClick={onClose}>Save</button>
                </div>
                {/*image container*/}
                <div className="w-[600px] h-[600px] flex items-center">
                    <NextImage
                        ...
                        height={600}
                        className={`w-full 
                        ${settings.type==="original" ? "h-full object-contain" : settings.type==="square" ? "aspect-square object-cover" : "aspect-video object-cover"}`}
                    />
                </div>
                {/*settings*/}
                <div className="flex items-center justify-between text-sm">
                    ...
                    <div
                        ...
                        onClick={()=>changeSensitive(!settings.sensitive)}>sensitive</div>
                </div>
            </div>
        </div>
    );
